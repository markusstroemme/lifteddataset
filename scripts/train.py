...
# finetuned_model_dir = os.path.join(script_dir, "..", "codebert-finetuned-vuldebert")
...
#         entry = self.samples[idx]
#         code = entry["code"]
#         embeddings = get_file_embeddings(code, strategy="cls")
#         num_chunks = embeddings.size(0)
#         return embeddings, torch.tensor(entry["label"], dtype=torch.long), entry["group"], num_chunks
...
# class LSTMClassifier(nn.Module):
#     def __init__(self, input_dim=768, hidden_dim=300, num_classes=2):
#         super().__init__()
...
if __name__ == "__main__":
    ...
    # Run both CV modes
    run_variant_cv(dataset, k=5, batch_size=8, epochs=100)
    run_sample_cv(dataset, k=5, batch_size=8, epochs=100)
